...
<body>
	...
	<script type="text/javascript">
			...
			initObject.Init = function() {
				$('#realTimeAlarmTable').bootstrapTable({
					...
					rowStyle: function (row, index) {
		                ...
		                var strclass = "";
		                if (row.alarmAckStatus.indexOf("未确认")!=-1) {
		                    strclass = 'danger';
		                }
		                if (row.alarmAckStatus.indexOf("已确认")!=-1) {
		                    strclass = 'success';
		                }
		    ...
		    oInit.Init = function () {
		        ...
		        $("#btn_confirm").click(function(){
		        	...
		        	if(selectRows.length <= 0){
		        		$.teninedialog({
		                    title:'操作提示',
		                    content:'<span style="font-size:15px;color:red;font-weight:bold;">请选择需要确认的告警！</span>'
		                });
		        		return;
		        	}else{
		        		//======确认的操作提示开始======
		        		$.teninedialog({
		                    title:'操作提示',
		                    content:'<span style="font-size:15px;color:red;font-weight:bold;">您确定需要确认选中的告警数据?</span>',
		                    ...
		                    clickButton:function(sender,modal,index){
		                    	if(index==0){
		        		        	...
		        		        	console.log("确认的告警:"+confirmAlarmDataArray);
		        		        	$.ajax({
		        						type: "POST",
		...
		function initMqttListener() {
			var ip = '192.168.1.133';
			var port = '8083';
			var clientId = 'RealTimeAlarmInfo';
			var topic = "/alarm/convertAlarmToMqtt/#";
			...
			function onMessageArrived(message) {
				...
				if(returnStr=='repeatAlarm'){
					for(var i=0;i<alarmDataArray.length;i++){
						if(mqttObj.no == alarmDataArray[i].alarmNumber){
							console.log("这是重复的告警数据");
							alarmDataArray[i].alarmHappenTime = mqttObj.last_change_time;
							alarmDataArray[i].alarmLastChangeTime = mqttObj.last_change_time;
						}
					}
